    ...
    <div className="options-strategy-simulator options-exec-simulator" style={{ color: "var(--options-sim-text)", fontSize: "0.85rem" }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--options-sim-label)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
            1. What is your view?
          </span>
          {selectedView && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
          {views.map((v) => (
            <button
              key={v.id}
              type="button"
              className={`sim-choice-btn ${v.type} ${selectedView === v.id ? "active" : ""}`}
              onClick={() => setSelectedView((prev) => (prev === v.id ? null : v.id))}
            >
              <div className="sim-choice-label">{v.label}</div>
              <div className="sim-choice-desc">{v.desc}</div>
            </button>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10 }}>
          {timeHorizons.map((h) => (
            <button
              key={h.id}
              type="button"
              className={`sim-choice-btn horizon ${selectedHorizon === h.id ? "active" : ""}`}
              onClick={() => setSelectedHorizon((prev) => (prev === h.id ? null : h.id))}
            >
              <div className="sim-choice-label">{h.label}</div>
              <div className="sim-choice-desc">{h.sub}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          {selectedView && selectedHorizon && (
            <span style={{ fontSize: "0.75rem", color: "var(--options-sim-subtle)" }}>
        ...
        {!selectedView || !selectedHorizon ? (
          ...
          <div className="table-scroll options-exec-table-scroll">
                      ...
                      >
                        <td>
                          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                            <span style={{ fontSize: "0.85rem", fontWeight: 600, color: isSelected ? "var(--options-sim-accent)" : "var(--options-sim-text)" }}>{s.name}</span>
                            <span style={{ fontSize: "0.75rem", color: "var(--options-sim-muted)" }}>{s.summary}</span>
                          </div>
                        </td>
                        <td>
                        ...
                        <td style={{ fontWeight: 600, color: "var(--options-sim-accent)" }}>{(s.probability * 100).toFixed(0)}%</td>
                        <td style={{ color: "var(--options-sim-text)" }}>{s.payoffLabel}</td>
                      </tr>
                      {isSelected && (
                        <tr key={`${s.id}-details`} className="strategy-details-row">
                          <td colSpan="4" style={{ padding: "16px", background: "var(--options-sim-details-bg)", borderBottom: "1px solid var(--options-sim-accent-border)" }}>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
                              {/* Explanation & Greeks */}
                              <div>
                                <div style={{ marginBottom: "12px", paddingBottom: "12px", borderBottom: "1px solid var(--options-sim-divider)" }}>
                                  <p style={{ fontSize: "0.8rem", color: "var(--options-sim-text)", margin: 0, lineHeight: 1.4, fontWeight: 500 }}>{s.summary}</p>
                                  <div style={{ fontSize: "0.72rem", color: "var(--options-sim-accent)", marginTop: "6px", fontStyle: "italic", background: "var(--options-sim-chip-bg)", padding: "4px 8px", borderRadius: "4px", display: "inline-block" }}>
                                    Structure: {s.legs}
                                  </div>
                                </div>
                                <div style={{ fontSize: "0.7rem", color: "var(--options-sim-label)", textTransform: "uppercase", marginBottom: "8px", letterSpacing: "0.05em" }}>Strategy Greeks (Live)</div>
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>
                                  <div className="greek-box" style={{ background: "var(--options-sim-greek-bg)", borderRadius: "6px", padding: "6px", textAlign: "center" }}>
                                    <label style={{ display: "block", fontSize: "0.6rem", color: "var(--options-sim-subtle)", marginBottom: "2px" }}>Delta</label>
                                    <strong style={{ fontSize: "0.85rem", color: "var(--options-sim-text)" }}>{realtimeGreeks.delta.toFixed(3)}</strong>
                                  </div>
                                  <div className="greek-box" style={{ background: "var(--options-sim-greek-bg)", borderRadius: "6px", padding: "6px", textAlign: "center" }}>
                                    <label style={{ display: "block", fontSize: "0.6rem", color: "var(--options-sim-subtle)", marginBottom: "2px" }}>Gamma</label>
                                    <strong style={{ fontSize: "0.85rem", color: "var(--options-sim-text)" }}>{realtimeGreeks.gamma.toFixed(4)}</strong>
                                  </div>
                                  <div className="greek-box" style={{ background: "var(--options-sim-greek-bg)", borderRadius: "6px", padding: "6px", textAlign: "center" }}>
                                  ...
                                  <div className="greek-box" style={{ background: "var(--options-sim-greek-bg)", borderRadius: "6px", padding: "6px", textAlign: "center" }}>
                                    ...
                                    <strong style={{ fontSize: "0.85rem", color: "var(--options-sim-accent)" }}>{realtimeGreeks.vega.toFixed(2)}</strong>
                                  </div>
                                </div>
                              </div>
                              ...
                              {/* Execution */}
                              <div>
                                <div style={{ fontSize: "0.7rem", color: "var(--options-sim-label)", textTransform: "uppercase", marginBottom: "8px", letterSpacing: "0.05em" }}>Trade Execution</div>
                                    ...
                                    <div style={{ position: "relative", flex: "1 1 120px" }}>
                                      {availableExpiries.length > 0 ? (
                                        <select
                                          ...
                                          style={{ width: "100%", padding: "8px", background: "var(--options-sim-input-bg)", border: "1px solid var(--options-sim-input-border)", borderRadius: "6px", color: "var(--options-sim-text)", fontSize: "0.85rem", appearance: "none" }}
                                        >
                                        ...
                                        <input
                                          ...
                                          onChange={(e) => setSelectedExpiry(e.target.value)}
                                          className="options-leg-input"
                                          style={{ width: "100%", padding: "8px", background: "var(--options-sim-input-bg)", border: "1px solid var(--options-sim-input-border)", borderRadius: "6px", color: "var(--options-sim-text)", fontSize: "0.85rem" }}
                                        />
                                      )}
                                      <span style={{ position: "absolute", right: "8px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: "var(--options-sim-accent)", fontSize: "14px" }}>
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                                      </span>
                                    </div>
                                    <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                                      <span style={{ position: "absolute", left: "8px", color: "var(--options-sim-label)", fontSize: "0.85rem" }}>$</span>
                                      <input 
                                        type="number"
                                        placeholder="Notional ($)"
                                        value={notionalDollars}
                                        onChange={(e) => setNotionalDollars(Number(e.target.value))}
                                        style={{ width: "90px", background: "var(--options-sim-input-bg)", border: "1px solid var(--options-sim-input-border)", borderRadius: "6px", padding: "8px 8px 8px 20px", color: "var(--options-sim-text)", fontSize: "0.85rem" }}
                                      />
                                    </div>
                                    <button
                                  ...
                                  {error && (
                                    <div style={{ 
                                      ...
                                      borderRadius: "6px",
                                      color: "var(--options-sim-error-text)",
                                      fontSize: "0.75rem",
                                      display: "flex",
                                      alignItems: "center",
                                      gap: "6px"
            ...
            <div style={{ marginTop: 6, fontSize: 10, color: "var(--options-sim-subtle)" }}>
