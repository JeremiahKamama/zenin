...
const EQUITIES_DATA_FETCH_ITEMS = [
  "Benchmark index history (daily, weekly, monthly, annual)",
  "Sector performance (tech, financials, healthcare, energy, industrials, consumer, utilities)",
  "Regional performance (USA, Europe, Asia, emerging markets, frontier markets)",
  "Style factors (value, growth, momentum, quality, low volatility, size)",
  "Rebalance signals (drift from target weights)",
  "Correlation matrix",
  "Volatility metrics (annualized vol, max drawdown, Sharpe, Sortino)",
  "Dividend data (yield, payout ratio, ex-dividend date, dividend growth)",
  "Earnings calendar (upcoming earnings, surprises, revisions)",
  "Valuation data (P/E, P/B, EV/EBITDA, dividend yield, FCF yield)",
  "Macro data (rates, inflation, unemployment, PMI, yield curve)",
  "Fund flows (ETF inflows/outflows by sector, region, asset class)",
  "FX rates",
  "Market breadth (A/D line, new highs/lows, % above moving averages)",
  "Risk indicators (VIX, credit spreads, treasury yields, liquidity indicators)",
  "Corporate actions (splits, buybacks, mergers, special dividends)",
];

const EQUITIES_PHASE_ROADMAP = [
  {
    phase: "Phase 1: Core Market Data",
    items: [
      "Live or delayed index performance panels",
      "Sector and regional performance tables",
      "Simple sparkline charts next to each benchmark",
      "Daily, YTD, 1Y, 3Y, 5Y, 10Y views",
      "Currency-aware display for each market",
    ],
  },
  {
    phase: "Phase 2: Portfolio Context",
    items: [
      "Correlation and diversification analysis",
      "Portfolio vs benchmark comparison",
      "Sector and country exposure views",
      "Risk metrics such as volatility and drawdown",
      "Factor exposure summaries",
    ],
  },
  {
    phase: "Phase 3: Decision Support",
    items: [
      "Earnings calendar and macro event calendar",
      "Valuation comparison across regions and sectors",
      "Relative strength ranking",
      "Market breadth indicators",
      "Flow-based signals for ETFs and funds",
    ],
  },
  {
    phase: "Phase 4: Premium Intelligence",
    items: [
      "Fund directory with AUM, fees, domicile, and structure",
      "Tax and withholding overlays by jurisdiction",
      "Rule-based alerts for valuation, volatility, or drawdown",
      "Scenario simulation for rate changes or macro shocks",
      "Watchlist alerts tied to thresholds",
    ],
  },
];

const EQUITIES_SOURCE_MODULES = [
  { module: "Benchmarks", focus: "Index levels, returns, drawdowns" },
  { module: "Sectors", focus: "Sector performance and rotation" },
  { module: "Regions", focus: "Country and regional performance" },
  { module: "Risk", focus: "Volatility, correlation, Sharpe, max drawdown" },
  { module: "Fundamentals", focus: "Valuation, earnings, margins" },
  { module: "Macro", focus: "Rates, inflation, PMIs, yields" },
  { module: "Funds", focus: "ETF flows, AUM, fees, structure" },
  { module: "FX", focus: "Exchange rates and trends" },
];

const EQUITIES_BUILD_ORDER = [
  "Benchmark + sector + regional performance",
  "Correlation and volatility metrics",
  "Fund directory enrichment (fees, AUM, domicile, structure)",
  "Valuation and earnings data",
  "Macro and rates data",
  "Flows, breadth, and alerts",
];

const EQUITIES_HIGHEST_VALUE_FETCHES = [
  "Sector performance",
  "Country performance",
  "Volatility",
  "Correlations",
  "Fund metadata (AUM and fees)",
];
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
                  <AnalyticsRoadmapCard
                    title="Data To Fetch"
                    subtitle="Equities module coverage targets"
                    items={EQUITIES_DATA_FETCH_ITEMS}
                  />
                  <AnalyticsRoadmapCard
                    title="Recommended Build Order"
                    subtitle="Implementation priority"
                    items={EQUITIES_BUILD_ORDER}
                    numbered
                  />
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20 }}>
                  {EQUITIES_PHASE_ROADMAP.map((phase) => (
                    <AnalyticsRoadmapCard
                      key={phase.phase}
                      title={phase.phase}
                ...
                <AnalyticsTableCard
                  title="Best Data Sources By Module"
                  subtitle="Source selection map for Equities analytics"
                  emptyText="No source-module rows."
                  columns={[
                    { key: "module", label: "Module" },
                    { key: "focus", label: "Coverage Focus" },
                  ]}
                  rows={EQUITIES_SOURCE_MODULES.map((row, idx) => ({ id: `src-${idx}`, ...row }))}
                />

                <AnalyticsRoadmapCard
                  title="Highest-value Next Fetches"
                  subtitle="Fastest impact additions"
                  items={EQUITIES_HIGHEST_VALUE_FETCHES}
                />
              </div>
            </>
          )}
...
function AnalyticsRoadmapCard({ title, subtitle, items = [], numbered = false }) {
  return (
    <div
      style={{
        background: "rgba(0, 0, 0, 0.85)",
      ...
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
        {items.length ? (
          items.map((item, idx) => (
            ...
            >
              <span style={{ color: "#7dd3fc", minWidth: 18 }}>
                {numbered ? `${idx + 1}.` : "•"}
              </span>
              <span>{item}</span>
            </div>
          ...
          <div style={{ color: "#64748b", fontSize: 12 }}>No items.</div>
        )}
      </div>
    </div>
  );
