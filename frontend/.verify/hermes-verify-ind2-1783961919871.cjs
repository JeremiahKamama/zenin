...
// .verify/stub2-1783961919871/chart.js
var require_chart = __commonJS({
  ".verify/stub2-1783961919871/chart.js"(exports2, module2) {
...
function IndicatorMetricModal({
  ...
  const HANDLER_FOR = {
    ...
    compare: onCompare ? () => onCompare(indicatorCode) : null,
...
(async () => {
  ...
  const journalBtn = Array.from(c1.querySelectorAll("button")).find((b) => b.textContent.trim() === "Journal");
  if (journalBtn) console.log("DEBUG journal btn:", JSON.stringify({ disabled: journalBtn.disabled, html: journalBtn.outerHTML.slice(0, 120) }), "calls:", JSON.stringify(calls.map((c) => c[0])));
