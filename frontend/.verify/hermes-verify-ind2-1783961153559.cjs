...
// .verify/stub2-1783961153559/chart.js
var require_chart = __commonJS({
  ".verify/stub2-1783961153559/chart.js"(exports2, module2) {
...
function IndicatorMetricModal({
  ...
  const HANDLER_FOR = {
    research: () => onOpenResearch && onOpenResearch(navArgs),
    profile: () => onOpenProfile && onOpenProfile(navArgs),
    watchlist: () => handleToggleWatch(),
    pin: () => onPin && onPin({ code: indicatorCode, label: metric2?.label }),
    alert: () => onAlert && onAlert({ code: indicatorCode, label: metric2?.label }),
    compare: () => onCompare && onCompare(indicatorCode),
    transmission: () => onOpenTransmission && onOpenTransmission(metric2?.label || indicatorCode),
    decisionLedger: () => onDecisionLedger && onDecisionLedger({ indicator: indicatorCode }),
    exposure: () => onExposure && onExposure({ indicator: indicatorCode }),
    export: () => onExport && onExport({ code: indicatorCode, label: metric2?.label, metric: metric2 }),
    copyLink: () => onCopyLink && onCopyLink({ code: indicatorCode, label: metric2?.label }),
    journal: () => A.onJournal && A.onJournal(navArgs),
    scenario: () => A.onScenario && A.onScenario(navArgs),
    macro: () => A.onMacroWorkspace && A.onMacroWorkspace(navArgs)
  };
